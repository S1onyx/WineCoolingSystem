...
void saveSetpoint() {
  ...
  if (isnan(lastSavedSetpoint) || fabsf(setpoint - lastSavedSetpoint) >= SP_SAVE_EPS) {
...
void maybePersistSetpoint(uint32_t now) {
  if (setpointDirty && (uint32_t)(now - lastSetpointChangeAt) >= SETPOINT_SAVE_DELAY_MS) {
    saveSetpoint();
  }
}
...
float applyCalibration(float raw) {
  if (isnan(raw) || CAL_COUNT <= 0) return raw;
  if (CAL_COUNT == 1) return raw + (CAL_REF[0] - CAL_RAW[0]);
...
void displayWake() {
  if (!displayIsOn) {
    // try-catch gibt’s nicht; wir zählen Fehler statt dessen
    if (!display.ssd1306_command(SSD1306_DISPLAYON)) {
      i2cErrorCount++;
    } else {
      displayIsOn = true;
    }
...
void displaySleep() {
  if (displayIsOn) {
    if (!display.ssd1306_command(SSD1306_DISPLAYOFF)) {
      i2cErrorCount++;
    } else {
      displayIsOn = false;
    }
...
void updateDisplayPower(uint32_t now) {
  if (displayIsOn && (uint32_t)(now - lastInteractionAt) >= DISPLAY_ON_TIMEOUT_MS) {
    displaySleep();
  }
}
...
Mode readModeSwitch(Mode lastKnown) {
  ...
  if (!a &&  b) return Mode::OFF;
  if ( a && !b) return Mode::ON;
  if ( a &&  b) return Mode::AUTO; // middle
...
void clampSetpoint() {
  setpoint = constrain(setpoint, SET_MIN, SET_MAX);
}

// ---------- AUTO helpers ----------
inline bool cooldownOver(uint32_t now) { return (uint32_t)(now - lastOpenAt) >= COOLDOWN_MS; }
inline bool shouldOpenNow() {
  if (isnan(lastTemp)) return false;
  return lastTemp > (setpoint + HYST * 0.5f);
}
void startOpen(uint32_t now)  { autoState = AutoState::OPEN;   stateStartedAt = now; lastOpenAt = now; relayWrite(true); }
void startClosed(uint32_t now){ autoState = AutoState::CLOSED; stateStartedAt = now;                   relayWrite(false); }
...
void controlLogicTimedAuto(bool modeJustEntered) {
  ...
  if (modeJustEntered) {
    ...
    startClosed(now);
    if (shouldOpenNow() && cooldownOver(now)) startOpen(now);
    return;
  }
  switch (autoState) {
    case AutoState::CLOSED:
      if (cooldownOver(now) && shouldOpenNow()) startOpen(now);
      break;
    case AutoState::OPEN: {
      bool timeUp = (uint32_t)(now - stateStartedAt) >= OPEN_DURATION_MS;
      if (timeUp || isnan(lastTemp)) startClosed(now);
      break;
    }
  }
}
void controlLogic(Mode mode, bool modeJustEntered) {
  switch (mode) {
    case Mode::OFF: startClosed(millis()); break;
    case Mode::ON:  relayWrite(true); autoState = AutoState::CLOSED; stateStartedAt = millis(); break;
    case Mode::AUTO: controlLogicTimedAuto(modeJustEntered); break;
  }
}
...
void drawSplash() {
  ...
  display.setCursor(0, 0);  display.println(F("Wine Cooling System"));
  display.setCursor(0,12);  display.println(F("designed by"));
  display.setCursor(0,22);  display.println(F("Simon Riedinger"));
  display.setCursor(0,36);  display.println(F("for"));
  display.setCursor(0,46);  display.println(F("Hannes Schock"));
  drawGrapeOutline(92, 8);
  if (!display.display()) { i2cErrorCount++; }
}
void drawMain(Mode m, float sp, float t, bool relay) {
  ...
  display.setCursor(0, 0); display.println(F("Controller"));

  display.setTextSize(1);
  display.setCursor(0, 20);
  display.print(F("Mode: "));
  switch (m) { case Mode::OFF: display.print(F("OFF")); break;
               case Mode::AUTO: display.print(F("AUTO")); break;
               case Mode::ON:  display.print(F("ON")); break; }
  ...
  if (isnan(t)) {
    display.print(F("--.-"));
    display.setCursor(80, 44); display.print(F("ERR"));
  } else {
    display.printf("%.1f", t);
  }
  display.print((char)247); display.print(F("C"));
...
bool handleButton(Btn &b, float delta) {
  ...
  float step = delta;
  if (held > 1200) step = (delta > 0 ? +1.0f : -1.0f);
  else if (held > 600) step = (delta > 0 ? +0.5f : -0.5f);
  ...
  if (!level && displayIsOn) {
    if (!b.handled && (uint32_t)(now - b.pressedAt) > 30) {
      setpoint += step; clampSetpoint(); b.handled = true; changed = true;
    }
    if ((uint32_t)(now - b.pressedAt) > 600 && (uint32_t)(now - b.lastRepeat) > 120) {
      setpoint += step; clampSetpoint(); b.lastRepeat = now; changed = true;
    }
  }
  b.lastLevel = level;

  if (changed) { setpointDirty = true; lastSetpointChangeAt = now; }
  return changed;
}
...
void setup() {
  ...
  if (hasAddr) sensors.setResolution(dsAddr, 12);
...
void loop() {
  ...
  if (modeJustEntered) displayWake();
  ...
  bool needRedraw =
      ...
      (!isnan(lastTemp) && !isnan(lastDrawnTemp) && fabsf(lastTemp - lastDrawnTemp) >= 0.1f) ||
      (fabsf(setpoint - lastDrawnSp) >= 0.1f) || (uint32_t)(now - lastInteractionAt) < 50;
